...
rearn = np.array([earnings[k]*(cpi[k]/cpi[-1]) for k in range(len(cpi))])
rprice = np.array([price[k]*(cpi[9 + k]/cpi[-1]) for k in range(N+1)])
realCumEarn = np.array([np.mean(rearn[k:k+W]) for k in range(len(cpi) - W + 1)])
inflation = np.diff(np.log(cpi))
nominalPrice = np.diff(np.log(price))
realPrice = nominalPrice - inflation[9:]
nominalTotal = np.array([np.log(price[k+1] + dividend[k+1]) - np.log(price[k]) for k in range(N)])
realTotal = nominalTotal - inflation[9:]
lvol = np.log(vol)
...
earnyield = realCumEarn[10-W:]/price

ngrowth = nearngr[9:]/vol
...
def simReturns(infl, nYears):
    ...
    simLVol = lvol[-1]
    simRet = np.array([])
    simYields = np.array([np.mean(simEarn[-W:])/simIndex[-1]])
    for t in range(nYears):
        simLVol = simLVol * betaVol + alphaVol + innov[t, 0]
        simVol = np.exp(simLVol)
        simGrowth = (innov[t, 1] + mean) * simVol
        simCumEarn = np.mean(simEarn[-W:])
        simEarn = np.append(simEarn, simEarn[-1] * np.exp(simGrowth))
        priceReturnsSim = (RegPrice.predict([1/simVol, 1, simYields[-1]/simVol]) + innov[t, 2]) * simVol
        simIndex = np.append(simIndex, simIndex[-1] * np.exp(priceReturnsSim))
        simRet = np.append(simRet, (RegTotal.predict([1/simVol, 1, simYields[-1]/simVol]) + innov[t, 3]) * simVol)
        simYields = np.append(simYields, simCumEarn/simIndex[-1])
    return simYields, simRet

def simWealth(infl, initialW, flow, horizon):
    simYield, returns = simReturns(infl, horizon)
...
def output(infl, initialW, flow, horizon):
    ...
    for sim in range(NSIMS):
        timeAvgRet, wealth, simYield = simWealth(infl, initialW, flow, horizon)
...
output(infl = 'R', initialW = 1000, flow = 0, horizon = 30)